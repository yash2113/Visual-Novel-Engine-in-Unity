...
namespace DIALOGUE.LogicalLines
{
    public static class LogicalLineUtils
    {
        ...
        public static class Expressions
        {
            ...
            public static void CalculateValue_DivisionAndMultiplication(List<string> operatorStrings, List<object> operands)
            {
            ...
            public static void CalculateValue_AdditionAndSubtraction(List<string> operatorStrings, List<object> operands)
            {
                for (int i = 0; i < operatorStrings.Count; i++) 
                {
                    string operatorString = operatorStrings[i];
        ...
        public static class Conditions
        {
            ...
            public static bool EvaluateCondition(string condition)
            {
                ...
                if(parts.Length == 1)
                {
                    ...
                    {
                        Debug.LogError($"Could not parse the condition: {condition}");
                        return false;
                    }
                }
            ...
            private static bool EvaluateExpression(string left, string op, string right)
            {
                ...
                switch(op)
                {
                    case "==": return left == right;
                        case "!=": return left != right;
                    default: throw new InvalidOperationException($"Unsupported Operation: {op} is not supported.");
                }
            }
