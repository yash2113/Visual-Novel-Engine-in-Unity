...
namespace TESTING
{
    public class CMD_DatabaseExtension_Examples : CMD_DatabaseExtension
    {
        new public static void Extend(CommandDatabase database)
        {
            ...
            database.AddCommand("lambda", new Action(() => { Debug.Log("Printing a default message to the console from lambda command."); }));
        ...
        {
            Debug.Log("Printing a default message to the console");
        }
        ...
        private static IEnumerator MoveCharacter(string direction)
        {
            ...
            while (Mathf.Abs(targetX - currentX) > 0.15f)
            {
